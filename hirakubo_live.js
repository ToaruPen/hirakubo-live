...
function mod2pi(x) { const m = x % TAU; return m < 0 ? m + TAU : m; }
function rhe(v) {            // numpy / Python round(): half to even
...
function vnH(u, v, s) {
  ...
  const a = x0 & 511, b = (x0 + 1) & 511, c = y0 & 511, d = (y0 + 1) & 511;
  return (hash3(a, c, s) * (1 - fx) + hash3(b, c, s) * fx) * (1 - fy)
       + (hash3(a, d, s) * (1 - fx) + hash3(b, d, s) * fx) * fy;
}
function fbmH(u, v, s, oct) {
  let sum = 0, amp = 1, norm = 0;
  for (let o = 0; o < oct; o++) { sum += amp * vnH(u * 2 ** o, v * 2 ** o, s + 101 * o); norm += amp; amp *= 0.5; }
  return sum / norm;
}
function n1(t, s) {           // smooth 1-D noise in [0, 1]
  const i = Math.floor(t); let f = t - i; f = f * f * (3 - 2 * f);
  return hash3(i | 0, 7, s) * (1 - f) + hash3((i + 1) | 0, 7, s) * f;
}

// ------------------------------------------------------------------ data
let D;
try { D = JSON.parse(document.getElementById('hk-data').textContent); }
catch (e) { fail('壁紙のデータを読み込めませんでした。ファイルが壊れていないか確認してください。'); return; }
const C = D.const;
const W = C.W, H = C.H, HY = C.HY, F = C.F, CAM_H = C.CAM_H, N = C.N, T = C.T, FPS = C.FPS;
const WIND = C.WIND, SHEEN = C.SHEEN, G = C.G;
const selfTest = [];
function dec(a) {
  const bin = atob(a.b64), u8 = new Uint8Array(bin.length);
  for (let i = 0; i < bin.length; i++) u8[i] = bin.charCodeAt(i);
  const Ty = { float64: Float64Array, float32: Float32Array, int16: Int16Array, uint16: Uint16Array,
               int32: Int32Array, uint8: Uint8Array }[a.dtype];
  const out = new Ty(u8.buffer);
  let s = 0; for (let i = 0; i < out.length; i++) s += out[i];
  if (Math.abs(s - a.sum) > 1e-6 * Math.max(1, Math.abs(a.sum))) selfTest.push(`array sum ${a.dtype} ${s} != ${a.sum}`);
  return out;
}
const A = {};
...
const SURF_Y = colRow(0), LAND_Y = colRow(1), KX_REEF = colRow(5), KX_ISLE = colRow(6);
const REEF_X = colRow(7), REEF_Z = colRow(8), ISLE_X = colRow(9);
const PAL_ORDER = ['sky', 'white', 'deep', 'lagoon', 'coral', 'grass', 'straw', 'rock'];
const palFlat = new Float32Array(57 * 3);
{ let o = 0; for (const k of PAL_ORDER) for (const c of D.pal[k]) { palFlat.set(c, o); o += 3; } }

// ------------------------------------------------------------------ WebGL2
const canvas = document.getElementById('hk');
const gl = canvas.getContext('webgl2', { antialias: false, alpha: false, depth: false, stencil: false,
                                          preserveDrawingBuffer: false, powerPreference: 'low-power' });
if (!gl) { fail('この壁紙の表示には WebGL2 が必要です。WebGL2 に対応したブラウザで開いてください。'); return; }
...
let P;
try {
  P = { base: program(FS_BASE), desp: program(FS_DESPECKLE), sea2: program(FS_SEA2),
        comp: program(FS_COMPOSE), grade: program(FS_GRADE), present: program(FS_PRESENT) };
} catch (e) { fail('描画の準備に失敗しました（シェーダー）。' + e.message); console.error(e); return; }
...
{
  const rows = D.cirrus.map(dec), buf = new Float32Array(512 * 96);
  let off = 0; for (const r of rows) { buf.set(r, off); off += r.length; }
...
function compSet(mode) {
  ...
  return { rough: c.rough, swell: c.swell, chop: c.chop, chopL: c.chopL, gust: c.gust, flutter: c.flutter };
}
...
function packUniform(rows, mode, i, t, out, gain = 1) {
  for (let j = 0; j < rows.length; j++) {
    ...
    out[4 * j] = r[0]; out[4 * j + 1] = r[1]; out[4 * j + 2] = r[2] * gain;
    out[4 * j + 3] = mod2pi(r[3] - tphase(r[4], mode, i, t) - (r[6] ? Math.PI / 2 : 0));
  }
  return out;
}
function pointSum(rows, X, Z, mode, i, t) {   // same sum as hl.Advected, at one point, in float64
  let s = 0;
  for (const r of rows) s += r[2] * Math.cos(r[0] * X + r[1] * Z - tphase(r[4], mode, i, t) + r[3]);
  return s;
}

// ------------------------------------------------------------------ surf timing per column
const SW_N = C.SWELL_N, SW_OM = TAU * SW_N / T, SET_AMP = C.SET_AMP;
const tauR = new Float64Array(W), ampR = new Float64Array(W), mR = new Int32Array(W);
const tauI = new Float64Array(W), ampI = new Float64Array(W);
function loopTiming(kx, i, tau, amp, mm) {    // hl.Surf.timing
  const ph = D.comps.loop.swell[0][3];
  const cyc = Math.floor(SW_N * i / N), frac = (SW_N * i) % N;
  for (let c = 0; c < W; c++) {
    const u = 2 * Math.PI * frac / N - kx[c] - ph;
    tau[c] = mod2pi(u) / SW_OM;
    const m = (((cyc + Math.floor(u / TAU)) % 3) + 3) % 3;
    amp[c] = SET_AMP[m]; if (mm) mm[c] = m;
  }
  return SW_OM;
}
...
function drawCaps(ev, count, ageOf) {
  const inten = new Float64Array(count), yy = new Float64Array(count), xx = new Float64Array(count),
        wp = new Float64Array(count);
  for (let j = 0; j < count; j++) {
    const e = ev(j), age = ageOf(j);
    const ride = Math.min(age, 0.8), along = CAP_C * ride + 0.15 * age;
    const X = e.x0 + (e.wx === undefined ? WIND[0] : e.wx) * along, Z = e.z0 + (e.wz === undefined ? WIND[1] : e.wz) * along;
    const y = HY + F * CAM_H / Z, x = W / 2 + X * F / Z;
    const wpx = e.size * (1 + 0.4 * Math.min(age, 1.5)) * F / Z;
    inten[j] = Math.min(1, age / 0.12) * Math.exp(-Math.max(age - 0.8, 0) / e.life)
             * clamp((wpx - 0.35) / 1.0, 0, 1) ** 1.5 * clamp((y - HY - 6) / 14, 0, 1);
    yy[j] = y; xx[j] = x; wp[j] = wpx;
  }
...
const loopCaps = { n: A.caps_z0.length,
  ev: j => ({ z0: A.caps_z0[j], x0: A.caps_x0[j], size: A.caps_size[j], life: A.caps_life[j] }) };
const liveCaps = {
  list: [], next: -10, rnd: mulberry32(4242), rate: 700 / 24,   // starts from a sea already breaking
  gauss() { const u = 1 - this.rnd(), v = this.rnd(); return Math.sqrt(-2 * Math.log(u)) * Math.cos(TAU * v); },
  step(t) {
    ...
    while (this.next <= t) {
      ...
      const ev = { z0, x0: (r() * 2.3 - 1.15) * z0 * (W / 2) / F, size: Math.exp(Math.log(5) + 0.5 * this.gauss()),
                   life: 1.2 + 1.8 * r(), birth: this.next };
      if (!env.ref) { const u = Math.hypot(mo.ux, mo.uz) || 1; ev.wx = mo.ux / u; ev.wz = mo.uz / u; }
      this.list.push(ev);
...
// ------------------------------------------------------------------ island surf
const ISL = C.ISLAND, ISLE_NOISE = A.isle_noise, ISLE_X0 = C.isle_x0;
function drawIsland() {
  const base = ISL.base + 1;
  const put = (x, y, code) => { if (x >= 0 && x < W && y >= 0 && y < H) over[(y * W + x) * 4 + 3] = code; };
  for (let x = ISLE_X0; x < ISL.x1 + 5; x++) {
    ...
    if (x < ISL.x0 + 6 && tauI[x] < 1.0 && n > 0.4) {
      for (let dy = 1; dy <= Math.trunc(3.5 * (1 - tauI[x]) * ampI[x]); dy++) put(x - 2, base - dy, 2);
    }
  }
}
...
const NT = TUFT.rx.length, NB = BLADE.b.length, NF = FG.x.length;
...
function plotR(x, y, v) { if (x >= 0 && x < W && y >= 0 && y < H) over[(y * W + x) * 4] = v; }
function plotG(x, y, v) { if (x >= 0 && x < W && y >= 0 && y < H) over[(y * W + x) * 4 + 1] = v; }
function drawBlades(comps, mode, i, t) {
  ...
  const tg = w ? mo.gustT : t, kg = w ? mo.kGust : 1, m = w ? mo.bend : 1;
  const lo = m >= 0 ? 0.05 : -2.2, hi = m >= 0 ? 2.2 : -0.05;
  for (let tu = 0; tu < NT; tu++) {            // tuft blades, then the dark root pixel
    const gx = TUFT.gx[tu], gz = TUFT.gz[tu];
    const gust = pointSum(comps.gust, gx, gz, mode, i, tg) * kg, flut = pointSum(comps.flutter, gx, gz, mode, i, tg) * kg;
    const base = clamp(rhe(TUFT.gv0[tu] + SHEEN * gust), 1, 6);
    const bend = clamp(m + 0.45 * gust + 0.25 * flut, lo, hi);
    const rx = TUFT.rx[tu], ry = TUFT.ry[tu];
    for (let b = tuftFirst[tu]; b < tuftFirst[tu + 1]; b++) {
      const n = BLADE.n[b], lean = BLADE.lean[b], b0 = BLADE.b[b];
      for (let k = 0; k < n; k++) {
        plotR(rhe(b0 + lean * bend * k * k / n), ry - k, Math.min(base + 1 + (k === n - 1 ? 1 : 0), 8) + 1);
      }
    }
  ...
  for (let f = 0; f < NF; f++) {               // foreground blades at the viewer's feet
    ...
    const n = FG.n[f], c = FG.c[f];
    for (let k = 0; k < n; k++) {
      const px = rhe(FG.x[f] + FG.le[f] * bend * k * k / n), py = FG.y[f] - k;
...
const UX = C.U_CLOUD * WIND[0], UZ = C.U_CLOUD * WIND[1], CB = C.CLOUD_BASE;
const LIGHT = (() => { const l = [-0.55, -0.7, 0.55], n = Math.sqrt(l[0] * l[0] + l[1] * l[1] + l[2] * l[2]); return l.map(v => v / n); })();
class Cloud {
  constructor(o) { Object.assign(this, o); this.sprite = null; this.drawnAt = -1; this.dx0 = mo.cloudX; this.dz0 = mo.cloudZ; }
  pose(t) {
    // in the still's wind the drift is U·(t − tb); otherwise it is the wind integrated since birth
    if (env.ref) {
      const dt = t - this.tb, Z = this.Z0 + UZ * dt, s = this.Z0 / Z;
      return { s, Z, ax: this.ax0 + (this.ax0 - W / 2) * (s - 1) + F * UX * dt / Z,
               base: this.base0 + (HY - this.base0) * (1 - s) };
    }
    const dx = mo.cloudX - this.dx0, Z = this.Z0 + mo.cloudZ - this.dz0, s = this.Z0 / Z;
    return { s, Z, ax: this.ax0 + (this.ax0 - W / 2) * (s - 1) + F * dx / Z,
             base: this.base0 + (HY - this.base0) * (1 - s) };
  }
  life(t) {
    ...
    return { size: (0.55 + 0.45 * u) * (1 - 0.35 * v),
             water: Math.min(this.grow ? smooth01((t - this.tb) / (0.6 * this.grow)) : 1, 1 - v) };
  }
  raster(t) {
    const { s, ax, base } = this.pose(t), { size, water } = this.life(t);
    const dx = ax - this.ax0, dy = base - this.base0, inv = 1 / s - 1;
    // env-mode kinds may flatten their puffs (ky), fray them (rag, baseRag) and shade darker
    const rag = this.rag ?? 1, br = this.baseRag ?? 1, vb = this.vb ?? 1.5, vs = this.vs ?? 5.0;
    let pf = this.puffs.map((q, j) => {
      const [cx, cy, r] = q;
      const g = 1 + 0.14 * (n1(t / this.evo[j][0] + this.evo[j][1], this.seed + j) - n1(this.evo[j][1], this.seed + j));
      return [cx + dx + (s - 1) * (cx - this.ax0), cy + dy + (s - 1) * (cy - this.base0), r * s * g * size, q[3] || 1];
    });
    if (this.kind === 'cb') {                          // only the turrets the rising tower has reached
      const lim = this.life(t).rise * this.hTop;
      pf = pf.filter((p, j) => this.base0 - this.puffs[j][1] <= lim);
      if (!pf.length) { this.sprite = null; this.shaft = null; return; }
    }
    let x0 = W, x1 = 0, y0 = H, y1 = 0, top = Infinity;
    for (const [cx, cy, r, ky] of pf) {
      x0 = Math.min(x0, Math.max(Math.trunc(cx - r - 3), 0)); x1 = Math.max(x1, Math.min(Math.trunc(cx + r + 4), W));
      y0 = Math.min(y0, Math.max(Math.trunc(cy - r - 3), 0)); y1 = Math.max(y1, Math.min(Math.trunc(cy + r + 4), H));
      top = Math.min(top, cy - r * ky);
    }
    this.shaft = this.rainK ? this.rasterShaft(t, ax, base, s, size) : null;
    if (x1 <= x0 || y1 <= y0 || y0 > HY) { this.sprite = null; return; }
    const gx0 = Math.max(x0 - 8, 0), gy0 = Math.max(y0 - 8, 0);
    const gw = Math.min(x1 + 8, W) - gx0, gh = Math.min(y1 + 8, H) - gy0;
    ...
    for (const [cx, cy, r, ky] of pf) {
      if (r <= 0) continue;
      const px0 = Math.max(Math.trunc(cx - r - 3), 0), px1 = Math.min(Math.trunc(cx + r + 4), W);
      const py0 = Math.max(Math.trunc(cy - r - 3), 0), py1 = Math.min(Math.trunc(cy + r + 4), H);
      for (let y = py0; y < py1; y++) for (let x = px0; x < px1; x++) {
        const gi = (y - gy0) * gw + (x - gx0);
        let nz = nzCache[gi];
        if (nz !== nz) nz = nzCache[gi] = noiseAt(x, y);
        const ax_ = (x + 0.5 - cx) / r, ay_ = (y + 0.5 - cy) / (r * ky);
    ...
    const band = 7 * s, LT = env.ref || !env.L ? LIGHT : env.L.light;
    for (let y = gy0; y < gy0 + gh && y <= HY; y++) for (let x = gx0; x < gx0 + gw; x++) {
      const gi = (y - gy0) * gw + (x - gx0);
      ...
      if (depth < 1) { idx[gi] = 8 + Math.round(48 * depth); continue; }  // lightens the sky by up to 3 steps
      const lx = x - gx0, ly = y - gy0;
      const gxv = lx === 0 ? bl[gi + 1] - bl[gi] : lx === gw - 1 ? bl[gi] - bl[gi - 1] : (bl[gi + 1] - bl[gi - 1]) / 2;
      const gyv = ly === 0 ? bl[gi + gw] - bl[gi] : ly === gh - 1 ? bl[gi] - bl[gi - gw] : (bl[gi + gw] - bl[gi - gw]) / 2;
  ...
  rasterShaft(t, ax, base, s, size) {
    ...
    const hw = 0.38 * this.wb * s * size, lean = this.lean, lf = this.life(t);
    const k = this.rainK * smooth01((lf.rise - 0.7) / 0.3) * lf.water;   // rain once the tower is deep
    if (k <= 0) return null;
    const ya = Math.max(Math.ceil(base + 1.5), 0), yb = Math.floor(HY);
    ...
    const sw = sx1 - sx0, sh = yb - ya + 1, d = new Uint8Array(sw * sh);
    for (let y = ya; y <= yb; y++) {
      const drop = y - base, xc = ax + lean * drop, onset = smooth01(drop / 4);
      for (let x = sx0; x < sx1; x++) {
        const u = (x + 0.5 - xc) / hw;
        const body = 1 - u * u + 0.6 * (vnH((x - xc) / 5 + 40, drop / 14 + t / 40, this.seed + 7) - 0.5);
        if (body <= 0) continue;
  ...
  fog(t) {
    const Z = this.pose(t).Z, V = env.wx.visibility;
...
function blur2(a, w, h) {       // hirakubo_pixel.blur(a, 2): box 5 twice per axis, edge padding
  let src = a, tmp = new Float64Array(w * h);
  for (let pass = 0; pass < 2; pass++) {
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      let s = 0; for (let d = -2; d <= 2; d++) s += src[y * w + clamp(x + d, 0, w - 1)];
      tmp[y * w + x] = s / 5;
    }
    const out = new Float64Array(w * h);
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      let s = 0; for (let d = -2; d <= 2; d++) s += tmp[clamp(y + d, 0, h - 1) * w + x];
      out[y * w + x] = s / 5;
    }
    src = out;
  }
...
function gridNoise(cd) {        // exported jitter noise of a still cloud, sampled bilinearly
  const [gx0, gy0, gw, gh] = cd.grid, nz = dec(cd.nz), edge = dec(cd.edge);
  return {
    noise(x, y) {
      const u = clamp(x - gx0, 0, gw - 1), v = clamp(y - gy0, 0, gh - 1);
      const i = Math.min(Math.floor(u), gw - 2), j = Math.min(Math.floor(v), gh - 2);
      const fx = u - i, fy = v - j;
      if (fx === 0 && fy === 0) return nz[j * gw + i];
      return (nz[j * gw + i] * (1 - fx) + nz[j * gw + i + 1] * fx) * (1 - fy)
           + (nz[(j + 1) * gw + i] * (1 - fx) + nz[(j + 1) * gw + i + 1] * fx) * fy;
    },
    edge(x) {
      const u = clamp(x - gx0, 0, gw - 1), i = Math.min(Math.floor(u), gw - 2), f = u - i;
      return f === 0 ? edge[i] : edge[i] * (1 - f) + edge[i + 1] * f;
    },
  };
}
function halfWidth(p, ax0) { return Math.max(...p.map(q => Math.abs(q[0] - ax0) + q[2])); }
function cumulus(x0, x1, base, height, rnd, tiers, lean) {   // hirakubo_pixel.cumulus
  const puffs = [], U = (a, b) => a + (b - a) * rnd();
  for (let tier = 0; tier < tiers; tier++) {
    const f = tier / Math.max(tiers - 1, 1);
    const a = x0 + (x1 - x0) * (0.18 * f + lean * f), b = x1 - (x1 - x0) * (0.38 * f - lean * f);
    const rr = height * (0.34 - 0.10 * f);
    for (let x = a; x <= b;) {
...
function tower(x0, x1, base, height, rnd, lean) {
  ...
  const puffs = [], w = x1 - x0, U = (a, b) => a + (b - a) * rnd();
  const rr = 0.18 * w, n = Math.max(2, Math.ceil(height / (0.8 * rr)));
  for (let k = 0; k < n; k++) {
    const f = k / (n - 1), cy = base - 0.5 * rr - f * (height - rr);
    if (cy + rr < -HY) break;                          // far above the frame
    const mid = (x0 + x1) / 2 + lean * f * w + U(-0.08, 0.08) * w, half = w * U(0.4, 0.53);   // bulging, uneven sides
    const a = mid - half + 0.6 * rr, b = mid + half - 0.6 * rr;
    for (let x = a; x <= b;) {
...
const sky = {
  clouds: [], rnd: mulberry32(9001), nextSpawn: 0, seed: 5000,
  evo(n) { return Array.from({ length: n }, () => [80 + 160 * this.rnd(), 100 * this.rnd()]); },
  init() {
    const r = this.rnd;
    this.clouds = D.clouds.map((cd, k) => {
      const p = cd.puffs, xs = p.map(q => q[0] - q[2]).concat(p.map(q => q[0] + q[2]));
      const ax0 = (Math.min(...xs) + Math.max(...xs)) / 2, base0 = cd.base;
      const Z0 = F * CB / (HY - base0);
      return new Cloud({ kind: 'cu', puffs: p, ax0, base0, Z0, tb: 0, half: halfWidth(p, ax0), seed: 700 + k, evo: this.evo(p.length),
                         decayAt: 900 + 1500 * r(), decayDur: 360 + 240 * r(), ...gridNoise(cd) });
    });
  ...
  spawn(t, mature, kind = 'cu') {
    ...
    const r = this.rnd, seed = this.seed++;
    const Z0 = Math.exp(Math.log(3000) + r() * Math.log(14000 / 3000));
    const xs = env.ref || mo.ux >= 0 ? -60 + 620 * r() : 700 - 620 * r(), base0 = HY - F * CB / Z0;
    const wpx = Math.min((500 + 1000 * r()) * F / Z0, 200);
    let hpx = wpx * (0.33 + 0.17 * r()), tiers = wpx > 60 ? 3 : 2;
    if (!env.ref) { const tall = env.wx.conv * r(); hpx *= 1 + 1.4 * tall; if (tall > 0.5 && wpx > 40) tiers = 4; }
    const puffs = cumulus(xs - wpx / 2, xs + wpx / 2, base0, hpx, mulberry32(seed), tiers, 0.12 * r() - 0.06);
    const c = new Cloud({
      kind: 'cu', puffs, ax0: xs, base0, Z0, tb: t, seed, half: halfWidth(puffs, xs), evo: this.evo(puffs.length), grow: 300 + 180 * r(),
      decayAt: t + 900 + 1500 * r(), decayDur: 360 + 240 * r(),
      noise: (x, y) => fbmH(x / 5, y / 5, seed, 3) - 0.5, edge: x => vnH(x / 7, 0, seed + 3),
    });
    if (!env.ref) c.thick = clamp((hpx * Z0 / F - 1500) / 6000, 0, 0.6);
    if (mature) { c.grow = 0; c.decayAt = t + 150 + 2200 * r(); }
    this.clouds.push(c);
  },
  spawnCb(t, mature) {
    ...
    const r = this.rnd, seed = this.seed++;
    const Z0 = 12000 + 18000 * r(), base0 = HY - F * CB / Z0;
    const xs = mo.ux >= 0 ? -80 + 660 * r() : 720 - 660 * r();
    const wpx = Math.min((3000 + 4000 * r()) * F / Z0, 260), hpx = (8000 + 4000 * r()) * F / Z0;
    const puffs = tower(xs - wpx / 2, xs + wpx / 2, base0, hpx, mulberry32(seed), 0.1 * r() - 0.05);
    const c = new Cloud({
      kind: 'cb', puffs, ax0: xs, base0, Z0, tb: t, seed, half: halfWidth(puffs, xs), evo: this.evo(puffs.length),
      grow: 600 + 400 * r(), decayAt: t + 1500 + 1500 * r(), decayDur: 600 + 300 * r(),
      wb: wpx, hTop: hpx, thick: 0.6, rainK: 0.7 + 0.25 * r(), lean: -0.2 * clamp(mo.ux / 8, -1.2, 1.2), rag: 0.7,
      noise: (x, y) => fbmH(x / 5, y / 5, seed, 3) - 0.5, edge: x => vnH(x / 7, 0, seed + 3),
    });
    if (mature) { c.grow = 0; c.decayAt = t + 300 + 2400 * r(); }
    this.clouds.push(c);
  },
  spawnFr(t, mature) {
    ...
    const r = this.rnd, seed = this.seed++;
    const hb = 150 + 250 * r(), Z0 = Math.max(hb * F / (HY - 20), 600 + 1900 * r()), base0 = HY - F * hb / Z0;
    const xs = mo.ux >= 0 ? -100 + 700 * r() : 740 - 700 * r();
    const wpx = Math.min((400 + 1400 * r()) * F / Z0, 260), hpx = wpx * (0.12 + 0.1 * r());
    const puffs = shreds(xs - wpx / 2, xs + wpx / 2, base0, hpx, mulberry32(seed));
    if (!puffs.length) return;
    const c = new Cloud({
      kind: 'fr', puffs, ax0: xs, base0, Z0, tb: t, seed, half: halfWidth(puffs, xs), evo: this.evo(puffs.length),
      grow: 60 + 90 * r(), decayAt: t + 240 + 420 * r(), decayDur: 120 + 120 * r(),
      rag: 2.2, baseRag: 3, vb: 0.4, vs: 2.2,
      noise: (x, y) => fbmH(x / 3, y / 3, seed, 3) - 0.5, edge: x => fbmH(x / 9, 0, seed + 3, 2),
    });
    if (mature) { c.grow = 0; c.decayAt = t + 60 + 500 * r(); }
    this.clouds.push(c);
  },
  step(t, live) {
    for (const c of this.clouds) {
      const ps = c.pose(t);
      if (ps.s < 0.35 && (c.decayAt == null || c.decayAt > t)) { c.decayAt = t; c.decayDur = 360 + 240 * this.rnd(); }
      // a cloud blown toward the viewer thins out before it grows past the frame
      if (!env.ref && ps.s > 1.8 && (c.decayAt == null || c.decayAt > t)) { c.decayAt = t; c.decayDur = 120 + 120 * this.rnd(); }
    }
    const ref = env.ref, right = ref || mo.ux >= 0;
    this.clouds = this.clouds.filter(c => {
      const ps = c.pose(t);
      const gone = c.decayAt != null && t > c.decayAt + c.decayDur;
      if (!ref && (ps.Z < (c.kind === 'fr' ? 400 : 1200) || ps.base < -40 || (!right && ps.ax + c.half * ps.s < -2))) return false;
  ...
  target() { return Math.round(env.wx.cu * 18); },
  targets() {
    const wx = env.wx, cb = Math.round(3 * clamp((wx.conv - 0.6) / 0.4, 0, 1) * clamp(wx.cu / 0.4, 0, 1));
    return { cu: Math.max(this.target() - cb, 0), cb, fr: Math.round(8 * wx.rain * wx.deck) };
  },
  PERIOD: { cu: 1500, cb: 2400, fr: 300 },
  next: { cu: 0, cb: 0, fr: 0 }, cull: { cu: 0, cb: 0, fr: 0 },
  nextCull: 0, dirty: false,
  regulate(t) {
    const tg = this.targets();
    for (const kind of ['cu', 'cb', 'fr']) {
      const n = tg[kind], all = this.clouds.filter(c => c.kind === kind);
      if (t >= this.next[kind]) {      // the further below its target, the sooner a kind forms again
        const deficit = n - all.length;
        if (deficit > 0) this.spawn(t, false, kind);
        this.next[kind] = t + (n === 0 ? 30 : Math.min(-this.PERIOD[kind] / n / Math.max(deficit, 1) * Math.log(1 - this.rnd()), 90));
      }
      if (t >= this.cull[kind]) {
        ...
        if (alive.length > n + (kind === 'cu' && n > 0 ? 1 : 0)) {
          ...
          old.decayAt = t; old.decayDur = (kind === 'fr' ? 60 : 240) + 240 * this.rnd();
          this.cull[kind] = t + (alive.length > n + 3 || n === 0 ? 15 : 45);
        }
      }
    }
  },
  reset(t, keepStill) {         // the population of a new weather, already grown (after a jump in time)
    const tg = this.targets();
    ...
    for (const kind of ['cb', 'cu', 'fr']) {
      ...
      for (let guard = 0; k < tg[kind] && guard < 40; guard++) { const m = this.clouds.length; this.spawn(t, true, kind); k += this.clouds.length > m ? 1 : 0; }
      this.next[kind] = t + 60; this.cull[kind] = t + 60;
    }
    this.nextSpawn = t + 60; this.nextCull = t + 60; this.dirty = true;
  },
  rasterDue(t, all) {           // one cloud per frame, round robin, unless a full redraw is asked
    if (this.dirty) { all = true; this.dirty = false; }
    if (all) { for (const c of this.clouds) { c.raster(t); c.drawnAt = t; } return; }
    let pick = null;
    for (const c of this.clouds) if (!pick || c.drawnAt < pick.drawnAt) pick = c;
    if (pick) { pick.raster(t); pick.drawnAt = t; }
  },
  composite(t) {
    ...
    for (const c of order) {
      ...
      if (sh) {
        for (let y = 0; y < sh.h; y++) for (let x = 0; x < sh.w; x++) {
          ...
          if (!a) continue;
          const k = ((sh.y0 + y) * W + sh.x0 + x) * 4, code = 4 + Math.round(a * (1 - fog) * 0.984);
          if (code > over[k + 2]) over[k + 2] = code;
        }
      }
      const sp = c.sprite; if (!sp) continue;
      for (let y = 0; y < sp.h; y++) {
        const yy = sp.y0 + y; if (yy > HY) break;
        for (let x = 0; x < sp.w; x++) {
          const v = sp.idx[y * sp.w + x];
          if (!v) continue;
          const k = (yy * W + sp.x0 + x) * 4, cur = over[k];
          if (v < 8 || cur === 0) { over[k] = v; if (w) over[k + 2] = fb; }  // a thin cloud never hides an opaque one
          else if (cur >= 8) { over[k] = Math.min(cur + v - 8, 255); if (w) over[k + 2] = fb; }
        }
      }
    }
  },
...
const bank = {
  v: F * C.U_CLOUD * WIND[0] / C.BANK_DIST, rnd: mulberry32(207207),
  bumps: D.bank.map(([x, w, h], k) => ({ p: x, w, h, tau: 300 + 600 * ((k * 0.61803) % 1), o: k * 13.7, s: 900 + k })),
  cx: 0, nb: 0,
  init() { this.cx = Math.min(...this.bumps.map(b => b.p)); },
  flow(shift) {                 // any wind: new heads appear upwind, old ones leave downwind
    if (!this.bumps.length) this.bumps.push({ p: W / 2 - shift, w: 10, h: 2, tau: 500, o: 0, s: 90000 + this.nb++ });
    let lo = Infinity, hi = -Infinity;
    for (const b of this.bumps) { lo = Math.min(lo, b.p); hi = Math.max(hi, b.p); }
    const r = this.rnd, add = () => {
      const w = 6 + 14 * r(), h = (1.5 + 1.7 * r()) * (w / 10) ** 0.8;
      const d = w * (0.45 + 0.35 * r()) + (r() < 0.28 ? 12 + 38 * r() : 0);
      return { w, h, d };
    };
    if (mo.ux >= 0) {
      if (lo + shift > W + 40) lo = W + 40 - shift;
      while (lo + shift > -30) { const n = add(); lo -= n.d; this.bumps.push({ p: lo, w: n.w, h: n.h, tau: 300 + 600 * r(), o: 100 * r(), s: 90000 + this.nb++ }); }
    } else {
      if (hi + shift < -40) hi = -40 - shift;
      while (hi + shift < W + 30) { const n = add(); hi += n.d; this.bumps.push({ p: hi, w: n.w, h: n.h, tau: 300 + 600 * r(), o: 100 * r(), s: 90000 + this.nb++ }); }
    }
    this.bumps = this.bumps.filter(b => b.p + shift - b.w / 2 < W + 60 && b.p + shift + b.w / 2 > -60);
  },
  step(t, live) {
    ...
    if (live && !env.ref) this.flow(shift);
      ...
      let refill = false;
      if (this.cx + shift > W + 40) { this.cx = W + 40 - shift; refill = true; }   // after a jump in time
      while (this.cx + shift > -30) {
        const r = this.rnd, w = 6 + 14 * r(), h = (1.5 + 1.7 * r()) * (w / 10) ** 0.8;
    ...
    for (const b of this.bumps) {
      ...
      const lo = Math.max(Math.ceil(cx - b.w / 2), 0), hi = Math.min(Math.floor(cx + b.w / 2), W - 1);
      for (let x = lo; x <= hi; x++) {
        const q = (x - cx) / (b.w / 2);
    ...
    for (let x = 0; x < W; x++) fl[x] = Math.floor(top[x] * clamp((Math.abs(x - C.LX) - 30) / 10, 0, 1) + 0.35);
    for (let x = 0; x < W; x++) {
      const dh = x === 0 ? fl[1] - fl[0] : x === W - 1 ? fl[W - 1] - fl[W - 2] : (fl[x + 1] - fl[x - 1]) / 2;
      colD[(W + x) * 4] = fl[x]; colD[(W + x) * 4 + 1] = dh;
    }
  },
};
...
const env = { ref: true, clock: 'now', speed: 1, force: null, wind: null, wetFix: null, light: 'beam', ms: Date.now(), wx: HE.REF_WEATHER,
              b: null, L: null, litMs: NaN, litReal: -Infinity, jump: false };
const LU = {
  sky: new Float32Array(150), cloud: new Float32Array(21), bank: new Float32Array(21), deck: new Float32Array(21),
  landG: new Float32Array([1, 1, 1]), reflG: new Float32Array([1, 1, 1]), haze: new Float32Array(3), vis: 60000,
  desat: 0, lampCol: new Float32Array([1, 0.9, 0.72]), lampLevel: 0, halo: 0, haloR: 4, rain: 0, beamK: 0, beamI: 0, beamB: 0,
  rainCol: new Float32Array(3), slant: 0, cirrusThr: 0.63, deckAmt: 0, deckH: 2000, scAmt: 0, scH: 900, starK: 0, mw: 0, mlim: -10,
  mwCol: new Float32Array(3), gal: new Float32Array(9),
  wet: 0, pool: 0, skyRefl: new Float32Array(3),
};
function refPalettes() {
  for (let a = 0; a < 5; a++) for (let l = 0; l < 10; l++) LU.sky.set(D.pal.sky[l], (a * 10 + l) * 3);
  for (let i = 0; i < 7; i++) for (const k of ['cloud', 'bank', 'deck']) LU[k].set(D.pal.white[i], i * 3);
  LU.cirrusThr = 0.63; LU.deckAmt = 0; LU.scAmt = 0; LU.starK = 0; LU.mw = 0; LU.lampLevel = 0; LU.rain = 0;
  LU.wet = 0; LU.pool = 0;
}
refPalettes();
...
const mo = { cloudX: 0, cloudZ: 0, deckX: 0, deckZ: 0, scX: 0, scZ: 0, scT: 0, bank: 0, cirrus: 0, gustT: 0, ux: UX, uz: UZ,
             kRough: 1, kSwell: 1, kCaps: 1, kGust: 1, bend: 1, gustTurn: NaN, seaTurn: NaN };
...
function turnTarget() {
  ...
  return Math.hypot(wv[0], wv[1]) > 0.3 ? Math.atan2(wv[0], wv[1]) - WIND_BEAR : Number.isFinite(mo.gustTurn) ? mo.gustTurn : 0;
}
function motionStep(dt, dts) {
  const wx = env.wx, wv = wx.windVec, U = wx.windSpeed, k = C.U_CLOUD / 8;
  mo.ux = wv[0] * k; mo.uz = wv[1] * k;
  mo.cloudX += mo.ux * dts; mo.cloudZ += mo.uz * dts;
  const P = 512 * 2600;
  mo.deckX = (mo.deckX + 1.3 * mo.ux * dts) % P; mo.deckZ = (mo.deckZ + 1.3 * mo.uz * dts) % P;
  mo.scX = (mo.scX + mo.ux * dts) % 512e3; mo.scZ = (mo.scZ + mo.uz * dts) % 512e3;   // stratocumulus: 512 cells of 1 km
  mo.scT += dts / 400;
  ...
  const ease = (a, tau, rate) => {
    ...
    return a + clamp(Math.atan2(Math.sin(d), Math.cos(d)) * (1 - Math.exp(-dts / tau)), -rate * dt, rate * dt);
  };
...
function eqVec(ra, dec) { ra *= D2R; dec *= D2R; return [Math.cos(dec) * Math.cos(ra), Math.cos(dec) * Math.sin(ra), Math.sin(dec)]; }
const GAL_X = eqVec(266.405, -28.936), GAL_Z = eqVec(192.8595, 27.1283);      // galactic centre, north galactic pole
const GAL_Y = [GAL_Z[1] * GAL_X[2] - GAL_Z[2] * GAL_X[1], GAL_Z[2] * GAL_X[0] - GAL_Z[0] * GAL_X[2], GAL_Z[0] * GAL_X[1] - GAL_Z[1] * GAL_X[0]];
function localFrame(lstDeg) {  // east, north, up of the cape in equatorial coordinates
  const t = lstDeg * D2R, p = HE.SITE.lat * D2R, ct = Math.cos(t), st = Math.sin(t), cp = Math.cos(p), sp = Math.sin(p);
  return { E: [-st, ct, 0], N: [-sp * ct, -sp * st, cp], U: [cp * ct, cp * st, sp] };
}
function galMatrix(lstDeg, out) {       // view (E, N, U) → galactic, column-major for uniformMatrix3fv
  const f = localFrame(lstDeg), cols = [f.E, f.N, f.U], rows = [GAL_X, GAL_Y, GAL_Z];
  for (let c = 0; c < 3; c++) for (let r = 0; r < 3; r++) out[c * 3 + r] = rows[r][0] * cols[c][0] + rows[r][1] * cols[c][1] + rows[r][2] * cols[c][2];
}
const linC = v => { v /= 255; return v <= 0.04045 ? v / 12.92 : ((v + 0.055) / 1.055) ** 2.4; };
// ------------------------------------------------------------------ wet ground
...
function wetStep(st, ms, force) {            // one grid step from ms, in the weather of its middle
  const mid = ms + WET.step / 2, wx = HE.weather(mid, force), h = WET.h;
  const R = wx.rain > 0.02 ? wx.rainRate : 0, alt = HE.bodies(mid).sun.app;
  const E = (0.03 + 0.55 * Math.max(Math.sin(alt * D2R), 0) * (1 - 0.75 * wx.deck)) * (0.6 + 0.08 * wx.windSpeed)
          * Math.max(0, 1 - R / 0.5);
...
function wetness(ms, force) {
  ...
  if (force !== wet.force || !(g >= wet.g) || g - wet.g > WET.back) {    // a jump: the whole day again
    const st = { w: 0, p: 0 };
  ...
  if (wet.next !== g) { const st = { w: wet.w, p: wet.p }; wetStep(st, g, force); wet.w1 = st.w; wet.p1 = st.p; wet.next = g; }
...
function updateEnv(realNow, force) {
  ...
  env.litMs = env.ms; env.litReal = realNow;
  const b = HE.bodies(env.ms), wx = HE.weather(env.ms, env.force);
  if (env.wind) {               // test hook: a fixed wind [m/s, from °] (the ground dries in the simulated one)
    const a = (env.wind[1] + 180) * D2R;
    wx.windSpeed = wx.seaWind = env.wind[0]; wx.windFrom = env.wind[1];
    wx.windVec = [Math.sin(a) * env.wind[0], Math.cos(a) * env.wind[0]];
  }
  const L = lightingOf(b, wx);
  env.b = b; env.wx = wx; env.L = L;
  LU.sky.set(L.skyPal); LU.cloud.set(L.cloudPal); LU.bank.set(L.bankPal); LU.deck.set(L.deckPal);
  LU.landG.set(L.landG); LU.reflG.set(L.reflG); LU.haze.set(L.hazeCol);
  LU.vis = wx.visibility; LU.desat = 0.45 * wx.deck;
  ...
  LU.scAmt = wx.deck * wx.sc < 0.02 ? 0 : wx.deck * wx.sc; LU.scH = 900;
  LU.deckAmt = wx.deck < 0.02 ? 0 : wx.deck * (1 - 0.6 * wx.sc * (1 - wx.deck)) * (1 + 0.3 * wx.rain);
  ...
  const sig = 3.912 / wx.visibility, sigR = 1.076 * (wx.rainRate || 0) ** 0.67 / 4343;
  ...
  LU.halo = 0.12 + 0.7 * wx.haze; LU.haloR = 2.5 + 6 * wx.haze + 4 * wx.rain;
  ...
  LU.wet = wt.w; LU.pool = wt.p;
  // what wet ground mirrors: the sky some 15° up toward the sea, or the cloud deck over it
  LU.skyRefl.set([0, 1, 2].map(c => lerp(linC(L.skyPal[63 + c]), linC(L.deckPal[9 + c]), wx.deck)));
}
// effects that run on real time whatever the sky's speed: the lamp's rhythm, rain, lightning
const fx = { lamp: 0, rainT: 0, flash: 0, glass: 1, haloK: 1, haloS: 1, beamK: 0, beamI: 0, phi: 0, a0: 0, a1: 0 };
...
const LANT = (() => { const z = F * C.EYE / (228 - HY); return [(213 - W / 2) * z / F, (HY - 121.5) * z / F, z]; })();
...
const beamOut = a => smooth01((Math.abs(Math.atan2(Math.sin(a - TO_EYE), Math.cos(a - TO_EYE))) - BEAM.screen) / BEAM.edge);
const bolt = { rnd: mulberry32(8080), next: 0, t0: -99, n: 0 };
function effects(t) {
  if (env.light === 'iso' || env.ref) {
    fx.lamp = env.ref ? 0 : LU.lampLevel * (t % 6 < 3 ? 1 : 0);    // 明3秒暗3秒
    fx.glass = 1; fx.haloK = 1; fx.haloS = 1; fx.beamK = 0; fx.beamI = 0;
  } else {
    ...
    fx.a0 = beamOut(fx.phi); fx.a1 = beamOut(fx.phi + Math.PI);
    fx.glass = 0.55; fx.haloK = 0.35; fx.haloS = 1;
    fx.beamK = LU.beamK * fx.lamp; fx.beamI = LU.beamI * fx.lamp;
  }
  fx.rainT = t % 600;
  const rate = env.ref ? 0 : env.wx.lightning / 12;
  if (rate <= 0) bolt.next = t + 5;
  else if (t >= bolt.next) { bolt.t0 = t; bolt.n = 1 + Math.floor(3 * bolt.rnd()); bolt.next = t - Math.log(1 - bolt.rnd()) / rate; }
  let f = 0;
  for (let k = 0; k < bolt.n; k++) { const d = t - bolt.t0 - 0.11 * k; if (d >= 0) f += Math.exp(-d / 0.045); }
...
const STARS = (() => {
  const n = 9000, r = mulberry32(31415), v = new Float64Array(3 * n), mag = new Float32Array(n), tint = new Uint8Array(n);
  for (let k = 0; k < n;) {
    const z = 2 * r() - 1, ph = TAU * r(), q = Math.sqrt(1 - z * z), x = q * Math.cos(ph), y = q * Math.sin(ph);
    const b = Math.asin(x * GAL_Z[0] + y * GAL_Z[1] + z * GAL_Z[2]);
    if (r() * 2.5 > 1 + 1.5 * Math.exp(-((b / 0.3) ** 2))) continue;
    v[3 * k] = x; v[3 * k + 1] = y; v[3 * k + 2] = z;
    mag[k] = 6.5 + 2 * Math.log10(1 - r());
    const u = r(); tint[k] = u < 0.4 ? 0 : u < 0.6 ? 1 : u < 0.85 ? 2 : 3;
    k++;
  }
  return { n, v, mag, tint };
})();
function drawStars(t) {
  ...
  const f = localFrame(env.b.lst), { E, N, U } = f, kext = 0.16 + 0.3 * env.wx.haze, S = STARS;
  for (let i = 0; i < S.n; i++) {
    const x = S.v[3 * i], y = S.v[3 * i + 1], z = S.v[3 * i + 2];
    const nn = N[0] * x + N[1] * y + N[2] * z;
    if (nn < 0.8) continue;
    const uu = U[0] * x + U[1] * y + U[2] * z;
    if (uu <= 0) continue;
    const ee = E[0] * x + E[1] * y + E[2] * z;
    const px = Math.floor(W / 2 + F * ee / nn), py = Math.round(HY + 0.5 - F * uu / nn);
    ...
    if (code > over[k + 1]) { over[k + 1] = code; over[k + 2] = S.tint[i]; }
  }
}

// ------------------------------------------------------------------ one frame
const U = { rough: new Float32Array(48), swell: new Float32Array(48), chop: new Float32Array(48),
            chopL: new Float32Array(48), gust: new Float32Array(48) };
...
for (const k of ['gust', 'flutter', 'rough', 'chop', 'chopL']) TURNED[k] = SETS.live[k].map(r => r.slice());
function turnRows(k, a) {
  const c = Math.cos(a), s = Math.sin(a), src = SETS.live[k], dst = TURNED[k];
  for (let j = 0; j < src.length; j++) {
    dst[j][0] = src[j][0] * c + src[j][1] * s; dst[j][1] = src[j][1] * c - src[j][0] * s;
  }
}
function turnWind() {
...
function simulate(mode, i, t, rasterAll, tc = t) {
  ...
  if (mode === 'loop') {
    ...
    if (w) for (let c = 0; c < W; c++) { ampR[c] = clamp(ampR[c] * mo.kSwell, 0.2, 1.8); ampI[c] = clamp(ampI[c] * mo.kSwell, 0.2, 1.8); }
    liveCaps.step(t);
    const L = liveCaps.list;
    drawCaps(j => L[j], L.length, j => t - L[j].birth);
  }
  const period = TAU / om;
  for (let c = 0; c < W; c++) {
  ...
  const gain = { rough: w ? mo.kRough : 1, swell: w ? mo.kSwell : 1, chop: w ? mo.kRough : 1, chopL: w ? mo.kRough : 1,
                 gust: w ? mo.kGust : 1 };
  for (const k of ['rough', 'swell', 'chop', 'chopL', 'gust']) packUniform(comps[k], mode, i, k === 'gust' && w ? mo.gustT : t, U[k], gain[k]);
  effects(t);
  cpuMs = performance.now() - c0;
  return { comps, lap: mode === 'loop' ? tphase(10, 'loop', i, t) : mod2pi(TAU * 10 / T * t),
...
function setComps(prog, name, key, comps) {
  ...
  if (prog.loc['uN' + name.slice(1)]) gl.uniform1i(prog.loc['uN' + name.slice(1)], comps[key].length);
}
function renderScene(frame) {
  ...
  const { comps } = frame;
  let pr = P.base; common(pr);
  bindTex(pr, 'uColS', 1, TX.colS); bindTex(pr, 'uColD', 2, TX.colD); bindTex(pr, 'uOver', 3, TX.over);
  bindTex(pr, 'uFieldA', 4, TX.fieldA); bindTex(pr, 'uFieldB', 5, TX.fieldB); bindTex(pr, 'uCirrus', 6, TX.cirrus);
  setComps(pr, 'uRough', 'rough', comps); setComps(pr, 'uSwell', 'swell', comps);
  setComps(pr, 'uChop', 'chop', comps); setComps(pr, 'uChopL', 'chopL', comps);
  gl.uniform1f(pr.loc.uCirrusShift, frame.shift);
  gl.uniform3fv(pr.loc.uSkyPal, LU.sky); gl.uniform3fv(pr.loc.uCloudPal, LU.cloud);
  gl.uniform3fv(pr.loc.uBankPal, LU.bank); gl.uniform3fv(pr.loc.uDeckPal, LU.deck);
  gl.uniform1f(pr.loc.uCirrusThr, LU.cirrusThr); gl.uniform1f(pr.loc.uDeck, LU.deckAmt);
  gl.uniform1f(pr.loc.uDeckH, LU.deckH); gl.uniform2f(pr.loc.uDeckOff, mo.deckX, mo.deckZ);
  gl.uniform1f(pr.loc.uSc, LU.scAmt); gl.uniform1f(pr.loc.uScH, LU.scH); gl.uniform1f(pr.loc.uScT, mo.scT);
  gl.uniform2f(pr.loc.uScOff, mo.scX, mo.scZ);
  gl.uniform1f(pr.loc.uStarK, LU.starK); gl.uniform1f(pr.loc.uMW, LU.mw);
  gl.uniform3fv(pr.loc.uMWCol, LU.mwCol); gl.uniformMatrix3fv(pr.loc.uGal, false, LU.gal);
  gl.uniform1f(pr.loc.uReflect, C.REFLECT); gl.uniform1f(pr.loc.uSwellReflect, C.SWELL_REFLECT);
  pass(pr, RT[0]);
  const desp = (src, dst, land) => {
    pr = P.desp; common(pr); bindTex(pr, 'uSrc', 1, src.t); gl.uniform1i(pr.loc.uLand, land); pass(pr, dst);
  };
  desp(RT[0], RT[1], 0); desp(RT[1], RT[0], 0);
  pr = P.sea2; common(pr);
  bindTex(pr, 'uSrc', 1, RT[0].t); bindTex(pr, 'uColS', 2, TX.colS); bindTex(pr, 'uColD', 3, TX.colD);
  bindTex(pr, 'uOver', 4, TX.over); bindTex(pr, 'uFieldA', 5, TX.fieldA); bindTex(pr, 'uFieldB', 6, TX.fieldB);
  bindTex(pr, 'uIsle', 7, TX.isle);
  setComps(pr, 'uGust', 'gust', comps);
  gl.uniform1f(pr.loc.uLap, frame.lap); gl.uniform1f(pr.loc.uBore, C.BORE);
  gl.uniform2i(pr.loc.uTrailY, C.trail_y[0], C.trail_y[1]);
  pass(pr, RT[1]);
  desp(RT[1], RT[0], 1); desp(RT[0], RT[1], 1);
  pr = P.comp; common(pr);
  bindTex(pr, 'uSrc', 1, RT[1].t); bindTex(pr, 'uOver', 2, TX.over); bindTex(pr, 'uFieldA', 3, TX.fieldA);
  bindTex(pr, 'uC1', 4, TX.c1); bindTex(pr, 'uC2', 5, TX.c2); bindTex(pr, 'uPost', 6, TX.post);
  setComps(pr, 'uGust', 'gust', comps);
  pass(pr, RT[0]);
  pr = P.grade; common(pr);
  bindTex(pr, 'uSrc', 1, RT[0].t);
  gl.uniform1i(pr.loc.uRef, env.ref ? 1 : 0);
  if (!env.ref) {
    gl.uniform3fv(pr.loc.uLandG, LU.landG); gl.uniform3fv(pr.loc.uReflG, LU.reflG); gl.uniform3fv(pr.loc.uHaze, LU.haze);
    gl.uniform1f(pr.loc.uVis, LU.vis); gl.uniform1f(pr.loc.uSeaDesat, LU.desat);
    gl.uniform3fv(pr.loc.uLampCol, LU.lampCol); gl.uniform1f(pr.loc.uLamp, fx.lamp);
    gl.uniform1f(pr.loc.uHalo, LU.halo); gl.uniform1f(pr.loc.uHaloR, LU.haloR);
    gl.uniform1f(pr.loc.uGlass, fx.glass); gl.uniform1f(pr.loc.uHaloK, fx.haloK); gl.uniform1f(pr.loc.uHaloS, fx.haloS);
    gl.uniform1f(pr.loc.uBeamK, fx.beamK); gl.uniform1f(pr.loc.uBeamB, LU.beamB); gl.uniform1f(pr.loc.uBeamPhi, fx.phi);
    gl.uniform1f(pr.loc.uBeamI, fx.beamI);
    gl.uniform2f(pr.loc.uBeamA, fx.a0, fx.a1);
    gl.uniform3f(pr.loc.uLant, LANT[0], LANT[1], LANT[2]);
    gl.uniform1f(pr.loc.uRain, LU.rain); gl.uniform3fv(pr.loc.uRainCol, LU.rainCol);
    gl.uniform1f(pr.loc.uRainT, fx.rainT); gl.uniform1f(pr.loc.uRainSlant, LU.slant);
    gl.uniform1f(pr.loc.uFlash, fx.flash);
    gl.uniform1f(pr.loc.uWet, LU.wet); gl.uniform1f(pr.loc.uPool, LU.pool);
    gl.uniform3fv(pr.loc.uSkyRefl, LU.skyRefl);
    bindTex(pr, 'uC1', 2, TX.c1); bindTex(pr, 'uC2', 3, TX.c2);
  }
  pass(pr, RT[1]);
}
...
function layout() {
  ...
  const cx = vw >= W ? W / 2 : clamp(Number.isFinite(pan.x) ? pan.x : Math.min(W / 2, 160 + vw / 2), vw / 2, W - vw / 2);
  const oy = vh >= H || !Number.isFinite(pan.y) ? (ch - H * s) * 0.55 : ch / 2 - clamp(pan.y, vh / 2, H - vh / 2) * s;
  view = { scale: s, ox: cw / 2 - cx * s, oy, cx, cy: (ch / 2 - oy) / s, panX: vw < W, panY: vh < H };
}
...
function setupPan() {
  ...
  const moveTo = (x, y) => {                          // keeps only an axis the screen crops
    const px = pan.x, py = pan.y;
    pan.x = x; pan.y = y; layout();
    pan.x = view.panX ? view.cx : px; pan.y = view.panY ? view.cy : py;
    present();
  };
  canvas.addEventListener('pointerdown', e => {
    if (e.pointerType === 'mouse' || drag) return;
    cancelAnimationFrame(fling);
    drag = { id: e.pointerId, x: e.clientX, y: e.clientY, cx: view.cx, cy: view.cy, moved: false, vx: 0, vy: 0, t: e.timeStamp };
  });
  canvas.addEventListener('pointermove', e => {
    ...
    const k = (window.devicePixelRatio || 1) / view.scale, x0 = view.cx, y0 = view.cy;
    moveTo(drag.cx - dx * k, drag.cy - dy * k);
    const dt = Math.max(e.timeStamp - drag.t, 1) / 1000, a = Math.min(dt / 0.05, 1);   // velocity, ~50 ms memory
    drag.vx += a * ((view.cx - x0) / dt - drag.vx); drag.vy += a * ((view.cy - y0) / dt - drag.vy);
    drag.t = e.timeStamp;
  });
  const end = e => {
    if (!drag || e.pointerId !== drag.id) return;
    ...
    let vx = d.vx, vy = d.vy, t0 = performance.now();
    const step = () => {
      const now = performance.now(), dt = Math.min((now - t0) / 1000, 0.05), x0 = view.cx, y0 = view.cy;
      t0 = now;
      moveTo(x0 + vx * dt, y0 + vy * dt);
      if (view.cx === x0) vx = 0;                     // stopped by an edge
      if (view.cy === y0) vy = 0;
      const f = Math.exp(-dt / 0.3); vx *= f; vy *= f;
...
function present() {
  ...
  const pr = P.present; gl.useProgram(pr.p);
...
async function loadImages() {
  for (const k in D.img) {
    ...
    const im = new Image();
    await new Promise((res, rej) => { im.onload = res; im.onerror = () => rej(new Error('image ' + k)); im.src = D.img[k].uri; });
...
const debug = location.hash === '#debug', testing = location.hash === '#test';
let simT = 0, simTc = 0, last = performance.now() / 1000, lastDraw = last, acc = 0, running = true, frames = 0, fpsT = 0, fpsShown = 0;
// a GPU reset (sleep, driver update) drops every texture; start over once the context is back
canvas.addEventListener('webglcontextlost', e => { e.preventDefault(); running = false; soundSync(); });
...
function advanceEnv(dtReal) {
  const prev = env.ms, expect = dtReal * env.speed * 1000;
...
function skySpeed() { return env.ref ? 1 : Math.min(env.speed, 60); }
function stepWorld(step, now) {
  simT += step;
  if (env.ref) { simTc = simT; return; }
  const sp = skySpeed();
  simTc += step * sp;
  const jumped = advanceEnv(step);
  updateEnv(now, jumped);
  motionStep(step, step * sp);
  if (jumped) { sky.reset(simTc, false); mo.gustTurn = mo.seaTurn = NaN; }
}
...
function tick() {
  ...
  last = now;
  if (HK_SETTINGS.fps > 0) {                         // documented Wallpaper Engine limiter
    acc += dt;
  ...
  present();
  if (snd.kit && snd.ctx.state === 'running') soundStep(snd.kit, snd.ctx.currentTime + SND.look, simT, step);
  frames++;
  if (now - uiT >= 0.25) { uiT = now; refreshUI(); }
  if (debug && now - fpsT >= 0.5) {
    fpsShown = frames / (now - fpsT); frames = 0; fpsT = now;
    hud.textContent = `fps ${fpsShown.toFixed(1)}  cpu ${cpuMs.toFixed(2)} ms\n` +
      `t ${simT.toFixed(1)} s  clouds ${sky.clouds.length}  caps ${liveCaps.list.length}  bank ${bank.bumps.length}\n` +
      (env.ref ? 'ref' : `${new Date(env.ms).toISOString().slice(0, 16)}Z ${env.wx.regime} sun ${env.b.sun.app.toFixed(1)}° r ${env.L.r.toExponential(1)} mlim ${LU.mlim.toFixed(1)}`);
  }
}
...
const JST = 9 * 3600e3;
const REGIME_JA = { clear: '快晴', fair: '晴れ', partly: '晴れ時々曇り', overcast: '曇り', showers: 'にわか雨', rain: '雨', storm: '荒天' };
const DIR16 = ['北', '北北東', '北東', '東北東', '東', '東南東', '南東', '南南東', '南', '南南西', '南西', '西南西', '西', '西北西', '北西', '北北西'];
const WDAY = '日月火水木金土';
const jst = ms => { const d = new Date(ms + JST); return { y: d.getUTCFullYear(), mo: d.getUTCMonth() + 1, d: d.getUTCDate(), wd: d.getUTCDay(), h: d.getUTCHours(), mi: d.getUTCMinutes() }; };
const hhmm = ms => { const p = jst(ms); return `${p.h}:${String(p.mi).padStart(2, '0')}`; };
const dayOf = ms => Math.floor((ms + JST) / 86400e3);
const rsCache = new Map();
function sunTimes(day) {
  if (!rsCache.has(day)) { if (rsCache.size > 8) rsCache.clear(); rsCache.set(day, HE.riseSet(day * 86400e3 - JST)); }
  return rsCache.get(day);
}
let uiT = 0, uiOn = false, dragging = false;
const PREF = 'hirakubo-sky-v1';
function savePrefs() {
  try {
    localStorage.setItem(PREF, JSON.stringify({ clock: env.clock, ms: env.clock === 'manual' ? env.ms : null, force: env.force, light: env.light,
                                                sound: snd.want, vol: snd.vol }));
...
function loadPrefs() {
  try {
    ...
    if (!o) return;
    if (o.clock === 'manual' && Number.isFinite(o.ms)) { env.clock = 'manual'; env.ms = o.ms; }
    if (o.force === null || (Number.isInteger(o.force) && o.force >= 0 && o.force < 7)) env.force = o.force;
...
function statusLines() {
  const p = jst(env.ms), wx = env.wx, b = env.b, day = dayOf(env.ms), rs = sunTimes(day);
  let sun;
  if (rs.rise && env.ms < rs.rise) sun = `日の出 ${hhmm(rs.rise)}`;
  else if (rs.set && env.ms < rs.set) sun = `日の入り ${hhmm(rs.set)}`;
  else { const n = sunTimes(day + 1); sun = `日の出 ${n.rise ? hhmm(n.rise) : '--'}`; }
  const wind = `${DIR16[Math.round(wx.windFrom / 22.5) % 16]}の風 ${Math.round(wx.windSpeed)} m/s`;
  const moon = `月齢 ${(b.moon.age * 29.53).toFixed(1)}`;
  return [`平久保崎 ${p.mo}月${p.d}日(${WDAY[p.wd]}) ${hhmm(env.ms)}${env.speed > 1 ? `　×${env.speed}` : ''}`,
          `${REGIME_JA[wx.regime]}${env.force == null ? '' : '（指定）'} ・ ${wind} ・ ${sun} ・ ${moon}`];
}
function setPressed(ids, on) { for (const id of ids) $(id).setAttribute('aria-pressed', String(id === on)); }
function syncManual() {
  const p = jst(env.ms);
  $('manual-date').value = `${p.y}-${String(p.mo).padStart(2, '0')}-${String(p.d).padStart(2, '0')}`;
...
function refreshUI() {
  ...
  $('sky-status').append(a, Object.assign(document.createElement('span'), { className: 'sub', textContent: b }));
  if (env.clock === 'manual') syncManual();
  soundHint();
}
function applyManual() {
  const d = $('manual-date').value.split('-').map(Number), m = Number($('manual-time').value);
  if (d.length !== 3 || d.some(v => !Number.isFinite(v))) return;
  env.ms = Date.UTC(d[0], d[1] - 1, d[2]) - JST + m * 60e3;
  env.jump = true; env.litMs = NaN; savePrefs(); refreshUI();
}
function setupUI() {
  if (HK_SETTINGS.wallpaper || typeof window.wallpaperRegisterAudioListener === 'function') return;
  uiOn = true;
  $('sky-ui').hidden = false;
  const panel = $('sky-panel'), toggle = $('sky-toggle');
  toggle.addEventListener('click', () => {
    panel.hidden = !panel.hidden; toggle.setAttribute('aria-expanded', String(!panel.hidden)); refreshUI();
  });
  const clockBtns = ['clock-now', 'clock-manual'], speedBtns = ['speed-1', 'speed-60', 'speed-600'];
  const showClock = () => { setPressed(clockBtns, env.clock === 'now' ? 'clock-now' : 'clock-manual'); $('manual-box').hidden = env.clock !== 'manual'; };
  $('clock-now').addEventListener('click', () => {
    env.clock = 'now'; env.ms = Date.now(); env.jump = true; env.litMs = NaN; showClock(); savePrefs(); refreshUI();
  });
  $('clock-manual').addEventListener('click', () => { env.clock = 'manual'; showClock(); syncManual(); savePrefs(); refreshUI(); });
  for (const id of speedBtns) $(id).addEventListener('click', () => {
    env.speed = Number(id.slice(6)); setPressed(speedBtns, id);
    if (env.clock === 'now' && env.speed === 1) { env.ms = Date.now(); env.jump = true; env.litMs = NaN; }
    refreshUI();
  });
  const range = $('manual-time');
  range.addEventListener('pointerdown', () => { dragging = true; });
  range.addEventListener('pointerup', () => { dragging = false; });
  range.addEventListener('input', () => { $('manual-out').value = `${Math.floor(range.value / 60)}:${String(range.value % 60).padStart(2, '0')}`; applyManual(); });
  ...
  for (const id of lightBtns) $(id).addEventListener('click', () => { env.light = id.slice(6); setPressed(lightBtns, id); savePrefs(); });
  ...
  const soundBtns = ['sound-off', 'sound-on'], vol = $('sound-vol');
  const showSound = () => { setPressed(soundBtns, snd.want ? 'sound-on' : 'sound-off'); $('sound-box').hidden = !snd.want; soundHint(); };
  for (const id of soundBtns) $(id).addEventListener('click', () => { snd.want = id === 'sound-on'; showSound(); soundSync(); savePrefs(); });
  vol.value = String(snd.vol);
  vol.addEventListener('input', () => { snd.vol = Number(vol.value); soundSync(); });
  ...
  const arm = () => { snd.armed = true; if (snd.want && !(snd.ctx && snd.ctx.state === 'running')) soundSync(); };
  for (const ev of ['pointerdown', 'pointerup', 'touchend', 'click', 'keydown']) window.addEventListener(ev, arm, true);
...
const ZONES = Array.from({ length: SND.zones }, (_, k) => {       // stretches of the reef, left to right
  const c = Math.round((k + 0.5) * W / SND.zones), x = REEF_X[c], z = REEF_Z[c];
...
const OWLS = [{ f: 850, pan: -0.5, lp: 4200, g: 1 }, { f: 805, pan: 0.65, lp: 2600, g: 0.45 }];   // a near male and a far one
const CICADAS = [{ f: 6300, pr: 88, pan: -0.6 }, { f: 6900, pr: 97, pan: 0.15 }, { f: 7400, pr: 104, pan: 0.7 }];
...
function noiseBuffers(ctx) {
  ...
  const sr = ctx.sampleRate, n = Math.round(8 * sr), m = Math.round(0.1 * sr), rnd = mulberry32(4242);
  ...
  for (let i = 0; i < n + m; i++) {
    ...
    raw[0][i] = x; raw[1][i] = b0 + b1 + b2 + b3 + b4 + b5 + b6 + x * 0.5362; b6 = x * 0.115926;
    br = (br + 0.02 * x) / 1.02; raw[2][i] = br;
  }
  return raw.map(r => {
    const buf = ctx.createBuffer(1, n, sr), d = buf.getChannelData(0);
    let s = 0;
    for (let i = 0; i < n; i++) { d[i] = i < m ? lerp(r[n + i], r[i], i / m) : r[i]; s += d[i] * d[i]; }
    const k = 0.3 / Math.sqrt(s / n);
    for (let i = 0; i < n; i++) d[i] *= k;
    return buf;
  });
}
function dropBuffer(ctx) {         // a drop on stone: a click that rings for a moment near 3 kHz
  const sr = ctx.sampleRate, n = Math.round(0.03 * sr), buf = ctx.createBuffer(1, n, sr), d = buf.getChannelData(0), rnd = mulberry32(77);
  for (let i = 0; i < n; i++) { const t = i / sr; d[i] = 0.8 * Math.sin(TAU * 3000 * t) * Math.exp(-t / 0.004) + 0.5 * (2 * rnd() - 1) * Math.exp(-t / 0.0008); }
  return buf;
}
// the whole graph, on any BaseAudioContext (the tests render it offline)
function soundKit(ctx) {
  const rnd = mulberry32(5150), [white, pink, brown] = noiseBuffers(ctx);
  ...
  const panner = p => { const s = ctx.createStereoPanner(); s.pan.value = p; return s; };
  const loop = buf => {
    const s = ctx.createBufferSource(); s.buffer = buf; s.loop = true;
    s.playbackRate.value = 0.97 + 0.06 * rnd(); s.start(0, rnd() * buf.duration);
    return s;
  };
  const out = gain(0), comp = ctx.createDynamicsCompressor();
  comp.threshold.value = -12; comp.knee.value = 6; comp.ratio.value = 8; comp.attack.value = 0.004; comp.release.value = 0.3;
  out.connect(comp).connect(ctx.destination);
  const bus = {};
  for (const k of ['surf', 'wind', 'rain', 'thunder', 'owl', 'cicada']) (bus[k] = gain(1)).connect(out);
  const K = { ctx, rnd, gain, filter, panner, white, pink, brown, drop: dropBuffer(ctx), out, bus, log: [],
              t: NaN, slow: -Infinity, bolt: bolt.t0, seen: new Float64Array(SND.zones) };
  // the reef: a bed from all of it and from the island's, and each stretch breaking in its turn
  K.bed = gain();
  for (const p of [-0.6, 0.6]) loop(pink).connect(filter('lowpass', 420, 0.5)).connect(panner(p)).connect(K.bed);
  loop(brown).connect(filter('lowpass', 110, 0.5)).connect(K.bed);
  K.bed.connect(bus.surf);
  K.zone = ZONES.map(Z => {
    const lp = filter('lowpass', 380, 0.5), g = gain();
  ...
  K.rumble = gain(); loop(brown).connect(filter('lowpass', 160, 0.5)).connect(K.rumble).connect(bus.wind);
  K.rustle = [-0.75, 0, 0.75].map(p => { const g = gain(); loop(white).connect(filter('bandpass', 3400, 0.45)).connect(g).connect(panner(p)).connect(bus.wind); return g; });
  K.whistleF = filter('bandpass', 500, 40); K.whistle = gain();
  ...
  K.hiss = gain();
  for (const p of [-0.7, 0.7]) loop(white).connect(filter('highpass', 1200, 0.5)).connect(panner(p)).connect(K.hiss);
  K.hiss.connect(bus.rain);
  K.patter = gain(); loop(pink).connect(filter('bandpass', 900, 0.6)).connect(K.patter).connect(bus.rain);
  // the owls: a near tone and a far one through the air's lowpass
  K.owlWave = ctx.createPeriodicWave(new Float32Array([0, 0, 0, 0]), new Float32Array([0, 1, 0.1, 0.03]));
  K.owlIn = OWLS.map(o => { const lp = filter('lowpass', o.lp, 0.7); lp.connect(panner(o.pan)).connect(bus.owl); return lp; });
  ...
  K.cic = CICADAS.map(c => {
    const am = gain(0.5), depth = gain(0.5), lvl = gain(), o = ctx.createOscillator();
    o.type = 'square'; o.frequency.value = c.pr; o.connect(depth).connect(am.gain); o.start();
    loop(white).connect(filter('bandpass', c.f, 4)).connect(am).connect(lvl).connect(panner(c.pan)).connect(bus.cicada);
    return { lvl, on: rnd() < 0.5, next: 5 * rnd() };
  });
  return K;
}
function note(K, e) { K.log.push(e); if (K.log.length > 200) K.log.shift(); }
function surfBreak(K, z, at, a) {
  const { lp, g } = K.zone[z], p = 0.16 * Math.min(a * a, 2.5) * 900 / ZONES[z].r;
...
function thunder(K, at, r, strokes) {
  ...
  const ctx = K.ctx, rnd = K.rnd, near = r < 2500;
  const D = 3 + 1.6 * r / 1000 + strokes, peak = 1.1 * clamp(1500 / r, 0.12, 1);
  const s = ctx.createBufferSource(), g = K.gain();
  s.buffer = K.brown; s.loop = true;
  s.connect(K.filter('lowpass', clamp(2000 * (1000 / r) ** 0.8, 110, 1200), 0.6)).connect(g).connect(K.bus.thunder);
  let tt = at;
  const bumps = 2 + strokes + Math.floor(3 * rnd());
  for (let k = 0; k < bumps; k++) {                     // the first arrival, then the rest of the channel
    const p = peak * (k === 0 ? 1 : 0.3 + 0.6 * rnd()), a = k === 0 && near ? 0.01 : 0.05 + 0.15 * rnd();
  ...
  s.start(at, rnd() * 6); s.stop(tt + D);
  if (near) {                                           // the crack of the first stroke
    const c = ctx.createBufferSource(), cg = K.gain();
    c.buffer = K.white; c.connect(K.filter('highpass', 700, 0.7)).connect(cg).connect(K.bus.thunder);
    cg.gain.setValueAtTime(0, at); cg.gain.linearRampToValueAtTime(0.7 * (2500 - r) / 1300, at + 0.004);
    cg.gain.setTargetAtTime(0, at + 0.004, 0.08);
    c.start(at, rnd() * 6); c.stop(at + 1);
  }
  note(K, { kind: 'thunder', at, r });
}
function owlCall(K, at, who, act) {
  const o = K.ctx.createOscillator(), e = K.gain(), f = OWLS[who].f, g = 0.1 * OWLS[who].g * act;
  ...
  o.start(at); o.stop(b + 0.3);
  note(K, { kind: 'owl', at, who });
}
const rainOf = wx => (wx.rain > 0.02 ? wx.rainRate : 0);          // mm/h, as the wet ground takes it
function owlAct() {                    // after dusk, out of the rain and a strong wind
  if (!env.b) return 0;
  return smooth01((-2 - env.b.sun.app) / 4) * clamp(1 - rainOf(env.wx), 0, 1) * clamp((12 - env.wx.windSpeed) / 4, 0, 1);
}
function cicadaAct() {                 // mid-March to early June, 7–12 h, dry, sun up, not too windy
  if (!env.b) return 0;
  const p = jst(env.ms), doy = (Date.UTC(p.y, p.mo - 1, p.d) - Date.UTC(p.y, 0, 1)) / 86400e3, h = p.h + p.mi / 60;
  const season = smooth01((doy - 74) / 16) * smooth01((166 - doy) / 20);
  const morning = smooth01((h - 6.5) / 1.5) * smooth01((12.5 - h) / 2);
  const wx = env.wx;
  return season * morning * (rainOf(wx) > 0.1 || env.b.sun.app < 0 ? 0 : 1) * (1 - 0.5 * wx.deck) * clamp((11 - wx.windSpeed) / 4, 0, 1);
}
// one step of the scene (sim time t, dt s) heard at context time `when`
function soundStep(K, when, t, dt) {
  const ctx = K.ctx, wx = env.wx, U = wx.windSpeed, R = rainOf(wx);
  const cont = t - K.t > 0 && t - K.t < 1;              // not the first step, nor a jump in time
  K.t = t;
  for (let z = 0; z < SND.zones; z++) {
    const Z = ZONES[z], n = crest.n[Z.c];
    if (cont && n === K.seen[z] + 1) surfBreak(K, z, when + Z.r / SND.c, ampR[Z.c]);
    K.seen[z] = n;
  }
  if (bolt.t0 !== K.bolt) {
    K.bolt = bolt.t0;
    if (cont && t - bolt.t0 < 1) { const r = 1200 + 7800 * K.rnd() ** 1.3; thunder(K, when + (bolt.t0 - t) + r / SND.c, r, bolt.n); }
  }
  if (R > 0) {                                          // drops on the stone, more as it rains harder
    const m = Math.min(4 + 2 * R, 40) * dt;
    for (let k = Math.floor(m) + (K.rnd() < m % 1 ? 1 : 0); k > 0; k--) {
      const s = ctx.createBufferSource();
      s.buffer = K.drop; s.playbackRate.value = 0.55 + 1.1 * K.rnd();
      s.connect(K.gain(0.02 + 0.07 * K.rnd() ** 2)).connect(K.panner(1.8 * K.rnd() - 0.9)).connect(K.bus.rain);
  ...
  let gm = 0;
  FG_EAR.forEach((f, k) => {
    const gn = clamp(pointSum(TURNED.gust, FG.gx[f], FG.gz[f], 'live', 0, mo.gustT) / (2 * GUST_RMS), -1, 1);
    gm += gn / 3;
  ...
  set(K.hiss.gain, 0.25 * rr, 1); set(K.patter.gain, 0.12 * rr, 1);
  // the owl calls in bouts of 6–23 calls some two seconds apart, then is quiet for a while
  const O = K.owl, act = owlAct();
  if (when + 0.15 >= O.next) {
    if (act < 0.05) { O.left = 0; O.next = when + 20 + 40 * K.rnd(); }
    else if (O.left === 0) { O.left = 6 + Math.floor(18 * K.rnd()); O.who = K.rnd() < 0.6 ? 0 : 1; }
    else {
  ...
  for (const c of K.cic) {
    if (when >= c.next) { c.on = !c.on; c.next = when + (c.on ? 20 + 70 * K.rnd() : 4 + 25 * K.rnd()); }
...
function soundSync() {
  ...
  const session = type => { try { if (navigator.audioSession) navigator.audioSession.type = type; } catch (e) { /* unsupported */ } };
  ...
  if (go && !snd.ctx && snd.armed) {
    ...
    } catch (e) { snd.ctx = snd.kit = null; snd.want = false; soundHint('このブラウザでは音を出せません。'); return; }
  }
  if (snd.ctx) {
    const g = snd.kit.out.gain, now = snd.ctx.currentTime;
    clearTimeout(snd.stop);
    if (go) {
      if (snd.ctx.state !== 'running') snd.ctx.resume().catch(() => {});
      g.cancelScheduledValues(now); g.setTargetAtTime((snd.vol / 100) ** 2, now, 0.4);
    } else {
      g.cancelScheduledValues(now); g.setTargetAtTime(0, now, 0.08);
      snd.stop = setTimeout(() => { if (snd.ctx.state === 'running') snd.ctx.suspend(); session('auto'); }, 500);
    }
  }
  soundHint();
}
function soundHint(text) {
  ...
  if (typeof text !== 'string') {
    text = '';
    if (snd.want && !running) text = '動きを止めている間は鳴りません。画面をタップ（クリック）すると動き出します。';
    else if (snd.want && !(snd.ctx && snd.ctx.state === 'running')) text = 'ブラウザの決まりで、画面を一度タップ（クリック）すると鳴り始めます。';
  }
...
(async () => {
  ...
  Object.assign(HKAPI, {
    selfTest,
    renderLoop(i) { renderScene(simulate('loop', i, i / FPS, true)); return readScene(); },
    renderLive(t) { renderScene(simulate('live', 0, t, true)); return readScene(); },
    advanceLive(t0, t1, step) {       // run the live systems forward without drawing every step
      for (let t = t0; t <= t1; t += step) { if (!env.ref) motionStep(step, step); simulate('live', 0, t); }
      let cover = 0;
      for (let x = 0; x < W; x++) cover += colD[(W + x) * 4] > 0;
      return { clouds: sky.clouds.length, caps: liveCaps.list.length, bank: bank.bumps.length, cover: cover / W };
    },
    cloudInfo: t => sky.clouds.map(c => ({ kind: c.kind, tb: c.tb, ...c.pose(t), ...c.life(t), fog: env.ref ? 0 : c.fog(t) })),
    beamInfo: () => ({ ...fx, beamKenv: LU.beamK, beamB: LU.beamB, X: env.L && env.L.X, lampLevel: LU.lampLevel, light: env.light }),
    skyState: () => ({ targets: env.ref ? null : sky.targets(), next: { ...sky.next }, cull: { ...sky.cull } }),
    rasterTimes: t => sky.clouds.map(c => { const a = performance.now(); c.raster(t); return [c.kind, +(performance.now() - a).toFixed(2), c.sprite ? c.sprite.w * c.sprite.h : 0]; }),
    // the sky of any moment and weather: { ref, time (UTC ms), weather (0–6 or null), speed, t, reset }
    setEnv(o = {}) {
      if ('time' in o) { env.clock = 'manual'; env.ms = o.time; }
      ...
      else { updateEnv(performance.now() / 1000, true); if (o.reset !== false) { sky.reset(o.t ?? simT, false); mo.gustTurn = mo.seaTurn = NaN; } }
      return HKAPI.envInfo();
    },
    envInfo: () => ({ ref: env.ref, ms: env.ms, regime: env.wx.regime, deck: env.wx.deck, cu: env.wx.cu, rain: env.wx.rain,
                      wind: [env.wx.windSpeed, env.wx.windFrom], sun: env.b && env.b.sun.app, r: env.L && env.L.r,
                      mlim: LU.mlim, lamp: LU.lampLevel, mo: { ...mo }, target: env.ref ? 6 : sky.target() }),
    wetInfo: () => ({ ...wetness(env.ms, env.force), ms: env.ms, wet: LU.wet, pool: LU.pool }),
    turnInfo: () => ({ gust: mo.gustTurn, sea: mo.seaTurn, target: env.ref ? 0 : turnTarget(),
                       rows: Object.fromEntries(['gust', 'rough', 'chop', 'chopL'].map(k => [k, TURNED[k].map(r => r.slice(0, 5))])) }),
    audioInfo: () => ({ want: snd.want, vol: snd.vol, armed: snd.armed, state: snd.ctx ? snd.ctx.state : 'none',
                        time: snd.ctx && snd.ctx.currentTime, owl: snd.kit && { ...snd.kit.owl }, owlAct: owlAct(), cicadaAct: cicadaAct(),
                        zones: ZONES, log: snd.kit ? snd.kit.log.slice(-30) : [] }),
    // the sound of the live scene rendered offline: { dur, dt, t0, solo: bus name, vol, owlAt, thunder: [[at, r, strokes]],
    // onStep(t, fx) }
    async soundTest(o = {}) {
      const sr = o.sr || 44100, dur = o.dur || 20, dt = o.dt || 0.05, t0 = o.t0 ?? simT;
      const ctx = new OfflineAudioContext({ numberOfChannels: 2, length: Math.ceil(sr * dur), sampleRate: sr });
    ...
    stop() { running = false; },
    setTime(t) { simT = t; },
    state: () => ({ t: simT, cpuMs, clouds: sky.clouds.length, caps: liveCaps.list.length, view: { ...view } }),
  });
  ...
  renderScene(simulate('live', 0, 0, true)); present();
  lastDraw = last = performance.now() / 1000;
  const reduce = window.matchMedia && matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (reduce) {                                       // show the scene still; a click starts it
    running = false;
    canvas.addEventListener('click', () => { if (!running) { running = true; last = lastDraw = performance.now() / 1000; soundSync(); tick(); } });
    window.addEventListener('resize', () => { present(); });
    return;
  }
  requestAnimationFrame(tick);
})().catch(e => { fail('描画を開始できませんでした。' + e.message); console.error(e); });
})();
